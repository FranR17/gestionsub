...
export function AuthScreen({
  ...
  const googleEnabled = hasSupabase && import.meta.env.VITE_AUTH_GOOGLE_ENABLED !== 'false'
  const appleEnabled = hasSupabase && import.meta.env.VITE_AUTH_APPLE_ENABLED === 'true'
  const hasOAuthProviders = googleEnabled || appleEnabled
  const authTitle = authMode === 'register' ? 'Crea tu espacio financiero' : 'Vuelve a tu control mensual'
  const authSubtitle = authMode === 'register'
    ? 'Guarda tus suscripciones, grupos y liquidaciones con sincronización segura.'
    : 'Revisa próximos cobros, pagos compartidos y presupuestos sin ruido.'

  return (
    <main className={`app-shell ${theme}`}>
      <section className="screen auth-screen">
        <div className="auth-brand">
          <div className="auth-logo" aria-hidden="true">N</div>
          <span className="auth-kicker">Notifyra</span>
          <h1>{authTitle}</h1>
          <p>{authSubtitle}</p>
          <div className="auth-highlights" aria-label="Ventajas principales">
        ...
        <form className="auth-form" onSubmit={(event) => void handleAuthSubmit(event)}>
          <div className="auth-form-head">
            <strong>{authMode === 'register' ? 'Alta sincronizada' : 'Acceso seguro'}</strong>
            <span>{hasSupabase ? 'Con Supabase activo' : 'Modo local disponible'}</span>
          </div>
          ...
          {isSyncing && <p className="auth-status" role="status">Conectando con Notifyra…</p>}
          {authError && <p className="form-err" role="alert">{authError}</p>}
          {authSuccess && <p className="form-ok" role="status">{authSuccess}</p>}
